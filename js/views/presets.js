...
RMTP.presets = (function () {
  ...
  const CONNECTOR_TYPES = [
    ...
    'speakON NL8',
    'powerCON TRUE1',
    'powerCON 20A Blue/Grey',
    'EtherCON (Cat6A / Dante)',
    'BNC (MADI / Wordclock)',
    'Multi-pin (Socapex / Harting)',
    'AES/EBU XLR'
  ];

  const DEFAULT_ROOM_IO_PRESETS = [
    {
      id: 'room-io-the-stage',
      space: 'The Stage',
      name: 'The Stage \u2014 Standard Live Production I/O',
      description: 'Standard multi-zone stagebox network with 32-channel digital trunk and 16-channel monitor & PA drive distribution.',
      homeRun: {
        name: 'The Stage \u2014 Main Stage Rack (DL32)',
        type: 'Digital Stage Rack (AES50 / Dante)',
        location: 'Stage Left Wing',
        inputChannels: 32,
        outputChannels: 16,
        prefix: 'HR',
        notes: 'Primary digital stage rack feeding FOH (dLive S7000) & Monitor console via redundant Dante.'
      },
      stageboxes: [
        {
          id: 'sb-stage-a',
          letter: 'A',
          name: 'Drum & Backline Sub-Snake',
          location: 'Upstage Centre',
          capacity: 8,
          channels: [
            { socket: 1, actId: 'act-house', instrument: 'Kick In', mic: 'Beta 91A', phantom: true, repatch: false, repatchTo: '', homeRunCh: 1, connectorType: 'XLR Female (Input)' },
            { socket: 2, actId: 'act-house', instrument: 'Kick Out', mic: 'Beta 52', phantom: false, repatch: false, repatchTo: '', homeRunCh: 2, connectorType: 'XLR Female (Input)' },
            { socket: 3, actId: 'act-house', instrument: 'Snare Top', mic: 'SM57', phantom: false, repatch: false, repatchTo: '', homeRunCh: 3, connectorType: 'XLR Female (Input)' },
            { socket: 4, actId: 'act-house', instrument: 'Hi-Hat', mic: 'C414', phantom: true, repatch: false, repatchTo: '', homeRunCh: 4, connectorType: 'XLR Female (Input)' },
            { socket: 5, actId: 'act-house', instrument: 'Rack Tom', mic: 'e604', phantom: false, repatch: false, repatchTo: '', homeRunCh: 5, connectorType: 'XLR Female (Input)' },
            { socket: 6, actId: 'act-house', instrument: 'Floor Tom', mic: 'e604', phantom: false, repatch: false, repatchTo: '', homeRunCh: 6, connectorType: 'XLR Female (Input)' },
            { socket: 7, actId: 'act-house', instrument: 'Percussion / Cajon', mic: 'Beta 91A', phantom: true, repatch: false, repatchTo: '', homeRunCh: 7, connectorType: 'XLR Female (Input)' },
            { socket: 8, actId: 'act-house', instrument: 'Drum Sub / Spare', mic: 'Radial ProDI', phantom: false, repatch: false, repatchTo: '', homeRunCh: 8, connectorType: 'XLR Female (Input)' }
          ],
          outputChannels: [
            { socket: 1, name: 'Drum Fill Sub Mix', type: 'Wedge', dest: 'Upstage Centre', connectorType: 'speakON NL4', homeRunOutCh: 4 },
            { socket: 2, name: 'Drummer Wired IEM / Click', type: 'IEM', dest: 'Drum Riser', connectorType: 'XLR Male (Output)', homeRunOutCh: 8 }
          ]
        },
        {
          id: 'sb-stage-b',
          ...
          capacity: 16,
          channels: [
            { socket: 1, actId: 'act-house', instrument: 'Brass / Horn 1', mic: 'Beta 57A', phantom: false, repatch: false, repatchTo: '', homeRunCh: 9, connectorType: 'XLR Female (Input)' },
            { socket: 2, actId: 'act-house', instrument: 'Tenor Sax', mic: 'DPA 4099', phantom: true, repatch: false, repatchTo: '', homeRunCh: 10, connectorType: 'XLR Female (Input)' },
            { socket: 3, actId: 'act-house', instrument: 'Alto Sax', mic: 'DPA 4099', phantom: true, repatch: false, repatchTo: '', homeRunCh: 11, connectorType: 'XLR Female (Input)' },
            { socket: 4, actId: 'act-house', instrument: 'Trombone', mic: 'MD421', phantom: false, repatch: false, repatchTo: '', homeRunCh: 12, connectorType: 'XLR Female (Input)' },
            { socket: 5, actId: 'act-house', instrument: 'Gtr SL', mic: 'SM57', phantom: false, repatch: false, repatchTo: '', homeRunCh: 13, connectorType: 'XLR Female (Input)' },
            { socket: 6, actId: 'act-house', instrument: 'Keys L', mic: 'ProD2', phantom: false, repatch: false, repatchTo: '', homeRunCh: 14, connectorType: 'XLR Female (Input)' },
            { socket: 7, actId: 'act-house', instrument: 'Keys R', mic: 'ProD2', phantom: false, repatch: false, repatchTo: '', homeRunCh: 15, connectorType: 'XLR Female (Input)' },
            { socket: 8, actId: 'act-house', instrument: 'Acoustic Gtr SL', mic: 'Radial ProDI', phantom: false, repatch: false, repatchTo: '', homeRunCh: 16, connectorType: 'XLR Female (Input)' },
            { socket: 9, actId: 'act-house', instrument: 'Backing Vox SL', mic: 'SM58', phantom: false, repatch: false, repatchTo: '', homeRunCh: 17, connectorType: 'XLR Female (Input)' },
            { socket: 10, actId: 'act-house', instrument: 'Talkback SL', mic: 'SM58 w/ Switch', phantom: false, repatch: false, repatchTo: '', homeRunCh: 18, connectorType: 'XLR Female (Input)' },
            { socket: 11, actId: 'act-house', instrument: 'Spare SL 1', mic: '', phantom: false, repatch: false, repatchTo: '', homeRunCh: null, connectorType: 'XLR Female (Input)' },
            { socket: 12, actId: 'act-house', instrument: 'Spare SL 2', mic: '', phantom: false, repatch: false, repatchTo: '', homeRunCh: null, connectorType: 'XLR Female (Input)' }
          ],
          outputChannels: [
            { socket: 1, name: 'Mix 1 (DSL Wedge)', type: 'Wedge', dest: 'Downstage Left', connectorType: 'speakON NL4', homeRunOutCh: 1 },
            { socket: 2, name: 'Mix 2 (SL Sidefill)', type: 'Fill', dest: 'Stage Left Wing', connectorType: 'speakON NL4', homeRunOutCh: 5 }
          ]
        },
        {
          id: 'sb-stage-c',
          ...
          capacity: 8,
          channels: [
            { socket: 1, actId: 'act-house', instrument: 'Bass DI', mic: 'ProDI', phantom: false, repatch: false, repatchTo: '', homeRunCh: 19, connectorType: 'XLR Female (Input)' },
            { socket: 2, actId: 'act-house', instrument: 'Bass Mic', mic: 'e906', phantom: false, repatch: false, repatchTo: '', homeRunCh: 20, connectorType: 'XLR Female (Input)' },
            { socket: 3, actId: 'act-house', instrument: 'Gtr SR', mic: 'e906', phantom: false, repatch: false, repatchTo: '', homeRunCh: 21, connectorType: 'XLR Female (Input)' },
            { socket: 4, actId: 'act-house', instrument: 'Lead Vox DSC', mic: 'Beta 58', phantom: false, repatch: false, repatchTo: '', homeRunCh: 22, connectorType: 'XLR Female (Input)' },
            { socket: 5, actId: 'act-house', instrument: 'Backing Vox SR', mic: 'SM58', phantom: false, repatch: false, repatchTo: '', homeRunCh: 23, connectorType: 'XLR Female (Input)' },
            { socket: 6, actId: 'act-house', instrument: 'Host / MC Wireless', mic: 'QLXD Handheld', phantom: false, repatch: false, repatchTo: '', homeRunCh: 24, connectorType: 'XLR Female (Input)' },
            { socket: 7, actId: 'act-house', instrument: 'Spare SR 1', mic: 'Radial ProDI', phantom: false, repatch: false, repatchTo: '', homeRunCh: null, connectorType: 'XLR Female (Input)' },
            { socket: 8, actId: 'act-house', instrument: 'Spare SR 2', mic: '', phantom: false, repatch: false, repatchTo: '', homeRunCh: null, connectorType: 'XLR Female (Input)' }
          ],
          outputChannels: [
            { socket: 1, name: 'Mix 3 (DSC Wedge)', type: 'Wedge', dest: 'Downstage Centre', connectorType: 'speakON NL4', homeRunOutCh: 2 },
            { socket: 2, name: 'Mix 4 (DSR Wedge)', type: 'Wedge', dest: 'Downstage Right', connectorType: 'speakON NL4', homeRunOutCh: 3 }
          ]
        }
      ]
    },
    {
      id: 'room-io-the-studio',
      space: 'The Studio',
      name: 'The Studio \u2014 Compact Live & Club I/O',
      description: '16 In / 8 Out setup with compact drop boxes tailored for intimate live gigs and club nights.',
      homeRun: {
        name: 'The Studio \u2014 Compact I/O Rack',
        type: 'Digital Stage Rack (AES50 / Dante)',
        location: 'Stage Left',
        inputChannels: 16,
        outputChannels: 8,
        prefix: 'STUDIO-HR',
        notes: 'Compact rack connecting Studio floor boxes to Yamaha QL1 console.'
      },
      stageboxes: [
        {
          id: 'sb-studio-a',
          letter: 'A',
          name: 'Front Stage Drop Snake',
          location: 'Centre Stage',
          capacity: 8,
          channels: [
            { socket: 1, actId: 'act-house', instrument: 'Vocal 1 (Lead)', mic: 'Beta 58A', phantom: false, repatch: false, repatchTo: '', homeRunCh: 1, connectorType: 'XLR Female (Input)' },
            { socket: 2, actId: 'act-house', instrument: 'Vocal 2', mic: 'SM58', phantom: false, repatch: false, repatchTo: '', homeRunCh: 2, connectorType: 'XLR Female (Input)' },
            { socket: 3, actId: 'act-house', instrument: 'Acoustic / Gtr DI', mic: 'Radial ProDI', phantom: false, repatch: false, repatchTo: '', homeRunCh: 3, connectorType: 'XLR Female (Input)' },
            { socket: 4, actId: 'act-house', instrument: 'Bass DI', mic: 'Radial ProDI', phantom: false, repatch: false, repatchTo: '', homeRunCh: 4, connectorType: 'XLR Female (Input)' },
            { socket: 5, actId: 'act-house', instrument: 'DJ Master L', mic: 'ProD2', phantom: false, repatch: false, repatchTo: '', homeRunCh: 5, connectorType: 'XLR Female (Input)' },
            { socket: 6, actId: 'act-house', instrument: 'DJ Master R', mic: 'ProD2', phantom: false, repatch: false, repatchTo: '', homeRunCh: 6, connectorType: 'XLR Female (Input)' },
            { socket: 7, actId: 'act-house', instrument: 'DJ Booth L', mic: 'Line In', phantom: false, repatch: false, repatchTo: '', homeRunCh: 7, connectorType: '1/4" TRS Jack' },
            { socket: 8, actId: 'act-house', instrument: 'DJ Booth R', mic: 'Line In', phantom: false, repatch: false, repatchTo: '', homeRunCh: 8, connectorType: '1/4" TRS Jack' }
          ],
          outputChannels: [
            { socket: 1, name: 'Studio Mix 1 (Wedge L)', type: 'Wedge', dest: 'Downstage Left', connectorType: 'speakON NL4', homeRunOutCh: 1 },
            { socket: 2, name: 'Studio Mix 2 (Wedge R)', type: 'Wedge', dest: 'Downstage Right', connectorType: 'speakON NL4', homeRunOutCh: 2 },
            { socket: 3, name: 'DJ Booth Monitor L', type: 'Wedge', dest: 'DJ Booth Left', connectorType: 'XLR Male (Output)', homeRunOutCh: 3 },
            { socket: 4, name: 'DJ Booth Monitor R', type: 'Wedge', dest: 'DJ Booth Right', connectorType: 'XLR Male (Output)', homeRunOutCh: 4 }
          ]
        }
      ]
    },
    {
      id: 'room-io-the-cinema',
      space: 'The Cinema',
      name: 'The Cinema \u2014 AV / Q&A Panel & Screening I/O',
      description: 'Lectern inputs, wireless lavaliers, surround cinema feeds, and stereo AV tie-lines.',
      homeRun: {
        name: 'The Cinema \u2014 AV Tie Rack',
        type: 'FOH Local Console I/O',
        location: 'FOH / Control',
        inputChannels: 12,
        outputChannels: 8,
        prefix: 'CIN-HR',
        notes: 'Dedicated tie lines between Cinema projection booth, stage podium, and AV processors.'
      },
      stageboxes: [
        {
          id: 'sb-cin-a',
          letter: 'A',
          name: 'Podium & Stage AV Drop',
          location: 'Downstage Right',
          capacity: 6,
          channels: [
            { socket: 1, actId: 'act-house', instrument: 'Lectern Gooseneck', mic: 'Condenser Gooseneck', phantom: true, repatch: false, repatchTo: '', homeRunCh: 1, connectorType: 'XLR Female (Input)' },
            { socket: 2, actId: 'act-house', instrument: 'Wireless Handheld 1', mic: 'QLXD Handheld', phantom: false, repatch: false, repatchTo: '', homeRunCh: 2, connectorType: 'XLR Female (Input)' },
            { socket: 3, actId: 'act-house', instrument: 'Wireless Handheld 2', mic: 'QLXD Handheld', phantom: false, repatch: false, repatchTo: '', homeRunCh: 3, connectorType: 'XLR Female (Input)' },
            { socket: 4, actId: 'act-house', instrument: 'Wireless Lav 1', mic: 'Bodypack / Lav', phantom: false, repatch: false, repatchTo: '', homeRunCh: 4, connectorType: 'XLR Female (Input)' },
            { socket: 5, actId: 'act-house', instrument: 'Laptop Audio L', mic: 'Radial USB-Pro', phantom: false, repatch: false, repatchTo: '', homeRunCh: 5, connectorType: '1/4" TRS Jack' },
            { socket: 6, actId: 'act-house', instrument: 'Laptop Audio R', mic: 'Radial USB-Pro', phantom: false, repatch: false, repatchTo: '', homeRunCh: 6, connectorType: '1/4" TRS Jack' }
          ],
          outputChannels: [
            { socket: 1, name: 'Cinema PA Left', type: 'Line Out', dest: 'Main Screen Left', connectorType: 'XLR Male (Output)', homeRunOutCh: 1 },
            { socket: 2, name: 'Cinema PA Right', type: 'Line Out', dest: 'Main Screen Right', connectorType: 'XLR Male (Output)', homeRunOutCh: 2 },
            { socket: 3, name: 'Cinema Sub', type: 'Line Out', dest: 'Cinema LFE Sub', connectorType: 'XLR Male (Output)', homeRunOutCh: 3 },
            { socket: 4, name: 'Assisted Hearing Loop', type: 'Feed', dest: 'Induction Loop Amp', connectorType: 'XLR Male (Output)', homeRunOutCh: 4 }
          ]
        }
      ]
    }
  ...
  /* ---- Room I/O Presets Store ---- */
  function getAllRoomIOPresets() {
    const venues = store.all('venues');
    return venues.filter(v => v.audio).map(v => ({
      id: 'room-io-' + v.id,
      space: v.name,
      name: v.name + ' \u2014 I/O Architecture',
      description: 'Permanent audio infrastructure for ' + v.name,
      homeRun: v.audio,
      stageboxes: (v.audio.stageboxes || []).map(sb => ({
        letter: sb.letter,
        name: sb.name,
  ...
  function getRoomIOPresetForSpace(spaceName) {
    const list = getAllRoomIOPresets();
    return list.find((p) => p.space === spaceName) || list[0] || null;
  }

  function saveRoomIOPreset(preset) {
    let venue = store.find('venues', preset.space);
    if (!venue) {
      venue = { id: preset.space, name: preset.space, dmx: [] };
    }
    
    venue.audio = JSON.parse(JSON.stringify(preset.homeRun || {}));
    if (preset.stageboxes) {
      venue.audio.stageboxes = preset.stageboxes.map(sb => ({
        letter: sb.letter,
        name: sb.name,
        location: sb.location,
        sockets: sb.channels ? sb.channels.length : 16
      }));
    }
    
    store.upsert('venues', venue);
    return venue;
  }

  function applyRoomIOPresetToSheet(sheet, roomPreset) {
    if (!sheet || !roomPreset) return;
    if (roomPreset.homeRun) {
      sheet.homeRun = JSON.parse(JSON.stringify(roomPreset.homeRun));
    }
    if (Array.isArray(roomPreset.stageboxes)) {
      sheet.stageboxes = JSON.parse(JSON.stringify(roomPreset.stageboxes));
      // Ensure acts are linked properly
      const defaultActId = (sheet.acts && sheet.acts[0]) ? sheet.acts[0].id : 'act-house';
      sheet.stageboxes.forEach((box) => {
        (box.channels || []).forEach((ch) => {
          if (!ch.actId || ch.actId === 'act-house') {
            ch.actId = defaultActId;
          }
        });
      });
    }
  ...
  function computeRepatchList(sheet) {
    ...
    sheet.stageboxes.forEach((box) => {
      const socketGroups = {};
      (box.channels || []).forEach(ch => {
        if (!socketGroups[ch.socket]) socketGroups[ch.socket] = [];
        socketGroups[ch.socket].push(ch);
      });

      Object.keys(socketGroups).forEach(socketNum => {
        const layers = socketGroups[socketNum];
        for (let i = 1; i < layers.length; i++) {
          const prev = layers[i-1];
          const curr = layers[i];
          const actPrev = (sheet.acts || []).find((a) => a.id === prev.actId) || {name: 'Unknown'};
          const actCurr = (sheet.acts || []).find((a) => a.id === curr.actId) || {name: 'Unknown'};
          out.push({
            stageboxLetter: box.letter || '?',
            stageboxName: box.name || '',
            stageboxLocation: box.location || '',
            socket: curr.socket,
            homeRunCh: curr.homeRunCh || null,
            fromAct: actPrev.name,
            toAct: actCurr.name,
            fromInst: prev.instrument || 'Line',
            toInst: curr.instrument || 'Line',
            actColor: actCurr.color || 'slate',
            mic: curr.mic || '',
            phantom: !!curr.phantom
          });
        }
      });
    });
  ...
  /* Helper to format and advance time (HH:MM) */
  function addMinutesToTime(timeStr, mins) {
    if (!timeStr || typeof timeStr !== 'string' || timeStr.indexOf(':') === -1) timeStr = '19:30';
  ...
  /* Helper to extract and synchronize artists from an Advancing Event into a Patch Sheet */
  ...
  function openPatchSheetModal(sheetOrNull, onSaved, initialStep) {
    ...
    // If an event is pre-linked and sheet acts are default or empty, auto-sync from schedule
    if (sheet.eventId) {
      const linkedEvent = events.find((e) => e.id === sheet.eventId);
      if (linkedEvent) {
        const hasCustomActs = sheet.acts.some((a) => a.id !== 'act-house' && a.name && a.name !== 'Headliner' && a.name !== 'Act 1');
        if (!hasCustomActs) {
          syncArtistsFromEvent(sheet, linkedEvent, true);
        }
      }
    }

    function applyVenueRoomIO(spaceName, updateDOM = false) {
    ...
    if (!isEdit) {
      applyVenueRoomIO(sheet.space);
    }

    let currentStep = initialStep ? Math.max(1, Math.min(5, parseInt(initialStep, 10))) : 1;
    ...
    const m = ui.modal({
        ...
        '<div class="grid gap-5 text-xs">' +
          ...
          '<div id="ps-step-1" class="' + (currentStep === 1 ? 'grid gap-5' : 'hidden') + '">' +
            ...
            '<div class="grid grid-cols-1 sm:grid-cols-3 gap-3 p-3.5 rounded-xl bg-panel2/40 border border-line">' +
              ...
              '</div>' +
              '<div>' +
                '<label class="block text-xs font-semibold mb-1">Linked Event (Bi-directional Sync)</label>' +
                '<select id="ps-event" class="field font-semibold text-accent">' +
                  '<option value="">(Standalone Patch Sheet \u2014 Not Linked)</option>' +
              ...
              '<div class="sm:col-span-3 mt-2">' +
                '<div class="flex items-center justify-between pb-2 border-b border-line/60 flex-wrap gap-2">' +
                  ...
                  '</div>' +
                  '<div class="flex items-center gap-2">' +
                    '<button type="button" id="btn-save-room-io" class="btn btn-ghost !py-1 !px-2.5 text-xs text-accent font-semibold flex items-center gap-1.5 border border-accent/30 hover:bg-accent/10" title="Pull Home Run & Stageboxes from Venue Settings">' +
            ...
            // Bi-directional Event & Schedule Synchronization Banner
            '<div id="ps-event-sync-banner"></div>' +
            ...
            '<div class="p-3.5 rounded-xl bg-panel border border-line flex flex-col gap-3 shadow-xs">' +
              '<div class="flex items-center justify-between pb-2 border-b border-line/60 flex-wrap gap-2">' +
                '<div class="flex items-center gap-2">' +
                  ...
                  '<span class="text-[11px] text-muted">(Used to color-code channel routing and generate event schedule items)</span>' +
                '</div>' +
          ...
          '<div id="ps-step-2" class="' + (currentStep === 2 ? 'grid gap-4' : 'hidden') + '">' +
            '<div class="flex items-center justify-between flex-wrap gap-2">' +
              ...
              '<div class="flex items-center gap-1.5 flex-wrap">' +
                ...
                '<button type="button" id="btn-add-local-sb" class="btn btn-ghost !py-1 !px-2.5 text-xs text-accent border border-line flex items-center gap-1 font-semibold" title="Add stagebox representing Local I/O from the Mixing Console">' +
            ...
            '<div class="flex items-start gap-3 p-3.5 bg-accent/5 border border-accent/20 rounded-lg text-xs">' +
              ...
              '<div>' +
                ...
                '<p class="text-muted leading-relaxed mb-1.5">For channels shared across all artists (like standard Drum Kits), set the Act to <strong class="text-ink">House / Venue Core</strong>.</p>' +
          ...
          '<div id="ps-step-4" class="' + (currentStep === 4 ? 'grid gap-5' : 'hidden') + '">' +
            ...
            '<div class="p-3.5 rounded-xl bg-panel border border-line flex flex-col gap-3 shadow-xs">' +
              '<div class="flex items-center justify-between pb-2 border-b border-line/60 flex-wrap gap-2">' +
                ...
                '</button>' +
              '</div>' +
              '<div id="ps-step-5" class="' + (currentStep === 5 ? 'grid gap-5' : 'hidden') + '">' +
            '<div class="p-3.5 rounded-xl bg-panel border border-line flex flex-col gap-3 shadow-xs">' +
              '<div class="flex items-center justify-between pb-2 border-b border-line/60 flex-wrap gap-2">' +
                '<div class="flex items-center gap-2">' +
      ...
      footer:
        ...
        '<div class="flex items-center gap-2">' +
          ...
          '<button type="button" id="btn-ps-next" class="btn btn-ghost text-xs font-semibold border border-line" style="' + (currentStep === 5 ? 'display:none;' : '') + '">Next Step &rarr;</button>' +
    ...
    function setStep(step) {
      ...
      if (prevBtn) {
        ...
        const prevLabels = ['', '', 'Event & Acts', 'Stageboxes & Drops', '⚡ Signal Flow', 'Repatches & Summary'];
    ...
    function renderEventSyncBanner() {
      const bannerEl = m.root.querySelector('#ps-event-sync-banner');
      if (!bannerEl) return;
      const evId = sheet.eventId;
      if (!evId) {
        bannerEl.innerHTML =
          '<div class="p-2.5 rounded-lg bg-panel2/40 border border-dashed border-line text-muted text-xs flex items-center justify-between gap-2">' +
            '<span class="flex items-center gap-1.5 font-medium">' + ui.icon('calendar', 'w-3.5 h-3.5 text-muted') + ' Select a linked event above to enable bi-directional artist and schedule synchronization.</span>' +
          '</div>';
        return;
      }
      ...
      const schedule = Array.isArray(ev.schedule_items) ? ev.schedule_items : (Array.isArray(ev.scheduleItems) ? ev.scheduleItems : []);
      const scheduleActs = schedule.filter((it) => it && (it.type === 'act' || (it.customName && it.customName.trim())));

      if (scheduleActs.length > 0) {
        bannerEl.innerHTML =
          '<div class="p-3 rounded-xl bg-accent/10 border border-accent/30 text-ink text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 shadow-2xs">' +
            '<div class="flex items-start gap-2.5">' +
              '<span class="text-accent shrink-0 mt-0.5">' + ui.icon('check', 'w-4 h-4') + '</span>' +
              '<div>' +
                '<div class="flex items-center gap-2 flex-wrap">' +
                  '<span class="font-bold text-accent">Event Advancing Linked:</span> ' +
                  '<span class="font-semibold">' + scheduleActs.length + ' Act' + (scheduleActs.length === 1 ? '' : 's') + ' in schedule</span>' +
                '</div>' +
                '<div class="text-[11px] text-muted flex items-center gap-1.5 flex-wrap mt-1">' +
                  scheduleActs.map((a) => '<span class="px-1.5 py-0.5 rounded bg-panel font-mono border border-line text-ink font-semibold flex items-center gap-1">' + ui.esc(a.customName || a.label || 'Act') + (a.time ? '<span class="text-accent font-bold">@' + ui.esc(a.time) + '</span>' : '') + '</span>').join('') +
                '</div>' +
              '</div>' +
            '</div>' +
            '<div class="flex items-center gap-1.5 shrink-0">' +
              '<button type="button" id="btn-pull-artists-from-event" class="btn btn-ghost !py-1 !px-2.5 text-xs text-accent font-semibold flex items-center gap-1 border border-accent/40 bg-panel hover:bg-accent/20 shadow-2xs">' +
                ui.icon('refresh', 'w-3.5 h-3.5') + '<span>Pull Artists from Schedule</span>' +
              '</button>' +
            '</div>' +
          '</div>';
      } else {
        bannerEl.innerHTML =
          '<div class="p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 text-ink text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 shadow-2xs">' +
            '<div class="flex items-center gap-2.5">' +
              '<span class="text-amber-500 shrink-0">' + ui.icon('info', 'w-4 h-4') + '</span>' +
              '<div>' +
                '<span class="font-bold text-amber-500">No Schedule Items in Event:</span> ' +
                '<span class="text-muted ml-1">Saving this patch sheet will automatically generate ' + Math.max(1, sheet.acts.filter((a) => a.id !== 'act-house').length) + ' schedule set piece(s) and changeovers in the Advancing schedule.</span>' +
              '</div>' +
            '</div>' +
            '<span class="text-[11px] font-mono px-2 py-0.5 rounded bg-panel border border-line text-muted shrink-0 font-semibold">Auto-generate on save</span>' +
          '</div>';
      }

      const pullBtn = bannerEl.querySelector('#btn-pull-artists-from-event');
      if (pullBtn) {
    ...
    function renderActs() {
      ...
      actsEl.innerHTML = sheet.acts.map((act, idx) => {
        ...
        const stageTimeBadge = act.stageTime ? '<span class="px-1.5 py-0.5 rounded text-[10px] font-mono font-bold bg-panel border border-line text-accent shadow-2xs" title="Stage time from Advancing schedule">' + ui.esc(act.stageTime) + '</span>' : '';
        return (
          '<div class="flex items-center gap-1.5 p-1.5 rounded-lg border ' + col.border + ' ' + col.bg + ' text-xs shadow-2xs">' +
    ...
    function renderStageboxes() {
          ...
          '<div class="panel p-4 rounded-xl border border-line bg-panel shadow-sm flex flex-col gap-3">' +
            ...
            '<div class="md:hidden grid gap-2.5">' +
              (!(box.channels && box.channels.length) ? (
                '<div class="text-center py-4 text-muted italic bg-panel2/30 rounded-lg border border-line p-3">No sockets in this stagebox. Click "Add Input Socket" above.</div>'
              ) : (
                socketGroups.map((grp) => {
                  ...
                  grp.layers.map((ch, layerIdx) => {
                    ...
                    '<div class="p-3 rounded-lg bg-panel border ' + (isRepatch ? 'border-warning/40' : 'border-line') + ' shadow-2xs grid gap-2 relative">' +
                      (isRepatch ? '<div class="absolute -left-[18px] top-[18px] text-accent/60">' + ui.icon('corner-down-right', 'w-4 h-4') + '</div>' : '') +
                      ...
                      '<div class="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 border-t border-line/60">' +
                        '<div>' +
                          '<label class="block text-[10px] uppercase font-bold text-muted mb-0.5">Connector</label>' +
                          '<select data-ch-conn="' + sbIdx + '-' + chIdx + '" class="field !py-1 !px-1.5 text-xs w-full">' +
                            CONNECTOR_TYPES.map((c) => '<option ' + (c === ch.connectorType ? 'selected' : '') + '>' + c + '</option>').join('') +
                          '</select>' +
                        '</div>' +
                        '<div>' +
              ...
              '<table class="w-full text-left text-xs border-collapse">' +
                '<thead>' +
                  '<tr class="border-b border-line bg-panel2/80 text-[10px] font-bold text-muted uppercase tracking-wider">' +
                    ...
                    '<th class="py-2 px-2.5 min-w-[130px]">Connector</th>' +
                ...
                '<tbody class="divide-y divide-line/40">' +
                  (!(box.channels && box.channels.length) ? (
                    '<tr><td colspan="9" class="text-center py-4 text-muted italic">No sockets in this stagebox. Click "Add Input Socket" above.</td></tr>'
                  ) : (
                    socketGroups.map((grp) => {
                      return grp.layers.map((ch, layerIdx) => {
                        const chIdx = ch._originalIndex;
                        const act = (sheet.acts || []).find((a) => a.id === ch.actId);
                        const col = act ? getActColorObj(act.color) : ACT_COLORS[0];
                        const hrCh = parseInt(ch.homeRunCh, 10);
                        const isRepatch = layerIdx > 0;
                        return (
                          '<tr class="hover:bg-panel transition-colors group ' + (isRepatch ? 'bg-panel2/50 relative' : '') + '">' +
                            // Skt Badge
                            ...
                            // Act Select
                            '<td class="py-2 px-2.5 align-middle relative">' +
                              (isRepatch ? '<div class="absolute left-0 top-0 bottom-0 w-0.5 bg-warning/50"></div>' : '') +
                            ...
                            // Connector
                            '<td class="py-2 px-2.5 align-middle">' +
                              '<select data-ch-conn="' + sbIdx + '-' + chIdx + '" class="field !py-0.5 !px-1.5 text-[11px] w-full">' +
                                CONNECTOR_TYPES.map((c) => '<option ' + (c === ch.connectorType ? 'selected' : '') + '>' + c + '</option>').join('') +
                              '</select>' +
                            '</td>' +
                            ...
                            // Repatch Button
                            '<td class="py-2 px-2.5 text-center align-middle">' +
                              '<button type="button" data-ch-duplicate="' + sbIdx + '-' + chIdx + '" class="text-accent hover:text-accent-ink p-1 rounded hover:bg-accent/10 transition-colors" title="Add Artist Changeover (Repatch)">' +
                            ...
                            // Delete Socket
                            '<td class="py-2 px-2 text-center align-middle">' +
                              '<button type="button" data-ch-del="' + sbIdx + '-' + chIdx + '" class="text-muted hover:text-danger p-1 rounded hover:bg-panel transition-colors" title="Remove socket layer">' +
      ...
      container.querySelectorAll('[data-ch-conn]').forEach((sel) => {
        sel.addEventListener('change', () => {
          const [sbIdx, chIdx] = sel.getAttribute('data-ch-conn').split('-').map(Number);
          sheet.stageboxes[sbIdx].channels[chIdx].connectorType = sel.value;
        });
      });
      ...
      container.querySelectorAll('[data-ch-del]').forEach((btn) => {
        btn.addEventListener('click', () => {
          ...
          // renumber unique sockets
          const channels = sheet.stageboxes[sbIdx].channels;
          let currentSocket = 1;
          let lastSocketSeen = -1;
          channels.forEach((c) => {
            if (c.socket !== lastSocketSeen) {
              lastSocketSeen = c.socket;
              c.socket = currentSocket++;
            } else {
              c.socket = currentSocket - 1;
            }
          });
          renderStageboxes();
        });
      });
      container.querySelectorAll('[data-ch-duplicate]').forEach((btn) => {
        btn.addEventListener('click', () => {
          const [sbIdx, chIdx] = btn.getAttribute('data-ch-duplicate').split('-').map(Number);
          const box = sheet.stageboxes[sbIdx];
          const sourceCh = box.channels[chIdx];
          const newCh = JSON.parse(JSON.stringify(sourceCh));
          // Find next act index
          const acts = sheet.acts || [];
          const currentActIdx = acts.findIndex(a => a.id === sourceCh.actId);
          if (currentActIdx >= 0 && currentActIdx < acts.length - 1) {
            newCh.actId = acts[currentActIdx + 1].id;
          }
          newCh.repatch = true;
          // Insert immediately after the source channel
          box.channels.splice(chIdx + 1, 0, newCh);
          renderStageboxes();
        });
      });
    }
    ...
    function renderRepatches() {
      ...
      // Group by Transition (From Act -> To Act)
      const transitions = {};
      repatches.forEach(r => {
        const key = r.fromAct + '|||' + r.toAct;
        if (!transitions[key]) transitions[key] = [];
        transitions[key].push(r);
      });

      summaryEl.innerHTML = Object.keys(transitions).map((key) => {
        const [fromAct, toAct] = key.split('|||');
        const items = transitions[key];
        
        return '<div class="mb-4 rounded-xl border border-line bg-panel2/50 overflow-hidden">' +
          '<div class="px-3 py-2 bg-panel2 border-b border-line flex items-center justify-between">' +
            '<div class="flex items-center gap-2">' +
              '<span class="font-bold text-ink text-xs uppercase tracking-wider">Changeover:</span>' +
              '<span class="px-2 py-0.5 rounded text-[10px] font-bold bg-panel border border-line text-muted">' + ui.esc(fromAct) + '</span>' +
              ui.icon('arrowR', 'w-3 h-3 text-muted') +
              '<span class="px-2 py-0.5 rounded text-[10px] font-bold bg-accent/15 border border-accent/30 text-accent">' + ui.esc(toAct) + '</span>' +
            '</div>' +
            '<span class="text-[10px] font-mono text-muted">' + items.length + ' operations</span>' +
          '</div>' +
          '<div class="p-2 grid gap-1.5">' +
            items.map(r => {
              const hrLabel = r.homeRunCh ? hrPrefix + ' ' + (r.homeRunCh < 10 ? '0' + r.homeRunCh : r.homeRunCh) : 'Unassigned';
              return (
                '<div class="p-2.5 rounded-lg bg-panel border border-line flex items-center justify-between flex-wrap gap-2 shadow-2xs">' +
                  '<div class="flex items-center gap-2 flex-wrap">' +
                    '<span class="font-mono font-bold text-warning text-[11px] px-2 py-0.5 rounded bg-warning/15 border border-warning/30">Box ' + r.stageboxLetter + ' Ch ' + r.socket + '</span>' +
                  ...
                  '</div>' +
                  '<div class="text-[11px] font-medium flex items-center gap-2 flex-1 justify-end">' +
                    '<span class="text-muted line-through">Unplug: ' + ui.esc(r.fromInst) + '</span>' +
                    ui.icon('arrowR', 'w-3.5 h-3.5 text-warning') + 
                    '<span class="text-ink font-bold">Plug In: ' + ui.esc(r.toInst) + (r.mic ? ' (' + ui.esc(r.mic) + ')' : '') + '</span>' +
                  '</div>' +
                '</div>'
              );
            }).join('') +
      ...
      for (let i = 0; i <= highestUsed; i++) {
        ...
        performingActs.forEach(act => {
           ...
           if (!actSocket) {
             ...
             const bgClass = isMuted ? 'opacity-30 grayscale hover:opacity-100 cursor-pointer' : 'hover:bg-accent/10 cursor-pointer';
    ...
    const addLocalBtn = m.root.querySelector('#btn-add-local-sb');
    if (addLocalBtn) {
    ...
    // Linked Event Selector Change Listener
    const evSelect = m.root.querySelector('#ps-event');
    if (evSelect) {
      evSelect.addEventListener('change', () => {
        const selectedId = evSelect.value || null;
        sheet.eventId = selectedId;
        if (selectedId) {
          const linkedEv = events.find((e) => e.id === selectedId);
          if (linkedEv) {
            sheet.eventName = linkedEv.name || '';
            if (linkedEv.space) {
            ...
            if (linkedEv.date) {
              sheet.date = linkedEv.date;
              const dtEl = m.root.querySelector('#ps-date');
              if (dtEl) dtEl.value = linkedEv.date;
            }
            const nameEl = m.root.querySelector('#ps-name');
            if (nameEl && (!nameEl.value || nameEl.value.trim() === 'Event Patch Sheet' || nameEl.value.trim() === 'New Event Patch Sheet')) {
    ...
    m.root.querySelector('#btn-save-sheet').addEventListener('click', () => {
      ...
      let linkedEv = null;
      if (evId) {
        linkedEv = events.find((e) => e.id === evId) || (store.all('advancing') || []).find((e) => e.id === evId);
        if (linkedEv) evName = linkedEv.name;
      }
      ...
      // Bi-directional Push to Event Advancing Schedule
      if (linkedEv) {
        let schedule = Array.isArray(linkedEv.schedule_items)
          ? JSON.parse(JSON.stringify(linkedEv.schedule_items))
          : (Array.isArray(linkedEv.scheduleItems) ? JSON.parse(JSON.stringify(linkedEv.scheduleItems)) : []);

        const realActs = sheet.acts.filter((a) => a && a.name && a.name.trim() && a.id !== 'act-house');
        const actsToSync = realActs.length > 0 ? realActs : sheet.acts.filter((a) => a && a.name && a.name.trim());

        if (schedule.length === 0 && actsToSync.length > 0) {
          // Event has NO schedule items: Create one per artist listed in patch sheet!
          const startBase = linkedEv.doors || linkedEv.startTime || '19:00';
          const newSchedule = [];

          actsToSync.forEach((act, idx) => {
            const actInputs = [];
            (sheet.stageboxes || []).forEach((b) => {
              (b.channels || []).forEach((c) => {
                if (c.actId === act.id && (c.instrument || c.mic)) {
                  actInputs.push({
                    channel: actInputs.length + 1,
                    instrument: c.instrument || 'Line',
                    mic: c.mic || 'Direct',
                    stand: 'Tall Boom',
                    pos: b.location || 'Centre Stage',
                    phantom: !!c.phantom
                  });
                }
              });
            ...
            const actTime = addMinutesToTime(startBase, 30 + idx * 60);
            newSchedule.push({
              type: 'act',
              label: 'Act ' + (idx + 1),
              customName: act.name,
              time: actTime,
              duration: '00:45',
              techReqType: actInputs.length ? 'channels' : 'none',
              techNotes: '',
              techFile: null,
              channelInputs: actInputs,
              channelOutputs: []
            });

            if (idx < actsToSync.length - 1) {
              const changeoverTime = addMinutesToTime(actTime, 45);
              newSchedule.push({
                type: 'changeover',
                label: 'Changeover',
                customName: 'Changeover (' + act.name + ' \u2192 ' + actsToSync[idx + 1].name + ')',
                time: changeoverTime,
                duration: '00:15',
                techReqType: 'none'
              });
          ...
          linkedEv.schedule_items = newSchedule;
          linkedEv.patch_sheet_id = saved.id;
          store.upsert('advancing', linkedEv);
          ui.toast('Created ' + actsToSync.length + ' schedule items for event "' + linkedEv.name + '"', 'ok');
        } else if (schedule.length > 0 && actsToSync.length > 0) {
          // Event has existing schedule items: Enrich blank act names and append any missing acts
          let modified = false;
          let actSlotIdx = 0;
          schedule.forEach((it) => {
            if (it.type === 'act') {
              if (!it.customName || !it.customName.trim()) {
                if (actsToSync[actSlotIdx]) {
                  it.customName = actsToSync[actSlotIdx].name;
                  modified = true;
                }
              }
              actSlotIdx++;
          ...
          actsToSync.forEach((act, aIdx) => {
            const found = schedule.some((it) => it.customName && it.customName.toLowerCase().trim() === act.name.toLowerCase().trim());
            if (!found && aIdx >= actSlotIdx) {
              const lastItem = schedule[schedule.length - 1];
              const lastTime = (lastItem && lastItem.time) ? lastItem.time : '21:00';
              schedule.push({
                type: 'act',
                label: 'Act ' + (schedule.filter((s) => s.type === 'act').length + 1),
                customName: act.name,
                time: addMinutesToTime(lastTime, 45),
                duration: '00:45',
                techReqType: 'none',
                techNotes: '',
                techFile: null,
          ...
          linkedEv.schedule_items = schedule;
          linkedEv.patch_sheet_id = saved.id;
          store.upsert('advancing', linkedEv);
          if (modified) {
            ui.toast('Synchronized patch sheet artists with Advancing schedule', 'ok');
          }
        } else {
          linkedEv.patch_sheet_id = saved.id;
          store.upsert('advancing', linkedEv);
  ...
  function printPatchSheet(sheet) {
    ...
    if (repatches.length) {
      ...
      sections.push({
        title: '3. STAGE CHANGEOVER & REPATCH CUE SHEET (' + repatches.length + ' OPERATIONS)',
        html: Object.keys(transitions).map((key) => {
          const [fromAct, toAct] = key.split('|||');
          const items = transitions[key];
          return '<div style="margin-bottom: 24px;">' +
            '<h4 style="margin: 0 0 8px 0; font-size: 14px; font-weight: bold; color: #334155; border-bottom: 2px solid #e2e8f0; padding-bottom: 4px;">' +
              'CHANGEOVER: ' + ui.esc(fromAct) + ' \u2192 ' + ui.esc(toAct) +
            '</h4>' +
            '<table>' +
              '<thead><tr><th style="width: 140px;">Socket</th><th style="width: 100px;">Home Run</th><th style="width: 150px;">Location</th><th>Unplug</th><th style="width: 30px;"></th><th>Plug In</th></tr></thead>' +
              '<tbody>' +
                items.map((r) => {
                  ...
                  return (
                    '<tr>' +
                      ...
                      '<td style="text-decoration: line-through; color: #64748b;">' + ui.esc(r.fromInst) + '</td>' +
                      '<td style="color: #b45309; text-align: center; font-weight: bold;">\u2192</td>' +
                      '<td style="font-weight: bold;">' + ui.esc(r.toInst) + (r.mic ? ' (' + ui.esc(r.mic) + ')' : '') + '</td>' +
                    '</tr>'
                  );
                }).join('') +
              '</tbody>' +
  ...
  function printDmxPatchSheet(title, meta, fixtures) {
    const fixList = Array.isArray(fixtures) ? fixtures : [];
    const val = RMTP.dmx ? RMTP.dmx.validatePatch(fixList) : { hasErrors: false, universeSummaries: {}, conflictMap: {} };
    const univKeys = Object.keys(val.universeSummaries || {}).sort((a, b) => +a - +b);

    const sections = [
      {
        title: 'DMX LIGHTING FIXTURE PATCH MATRIX (' + fixList.length + ' FIXTURES)',
        badge: univKeys.length ? univKeys.map((u) => 'Univ ' + u + ': ' + val.universeSummaries[u].totalChannelsUsed + '/512ch').join(' | ') : 'No Universes',
        html:
          '<table>' +
            '<thead><tr><th style="width: 35px;">Unit</th><th style="width: 170px;">Fixture Personality / Profile</th><th>Custom Name / Purpose</th><th style="width: 120px;">Hanging Location</th><th style="width: 45px;">Univ</th><th style="width: 55px;">Start</th><th style="width: 55px;">End</th><th style="width: 45px;">Ch</th></tr></thead>' +
            '<tbody>' +
              (fixList.length ? (
                fixList.map((f, idx) => {
                  ...
                  const end = addr + ch - 1;
                  const fixId = f.id || 'fix-' + idx;
                  const isConflict = val.conflictMap && val.conflictMap[fixId];
                  return (
                    '<tr class="' + (isConflict ? 'collision' : '') + '">' +
                      '<td style="font-weight: bold; font-family: monospace;">#' + (f.unit !== undefined ? f.unit : (idx + 1)) + '</td>' +
                      '<td><b>' + ui.esc((f.manufacturer ? f.manufacturer + ' ' : '') + (f.model || 'Fixture')) + '</b><div style="font-size: 9px; color: #64748b;">' + ui.esc(f.mode || 'Standard') + '</div></td>' +
                      '<td>' + ui.esc(f.name || f.model || '\u2014') + '</td>' +
                      '<td>' + ui.esc(f.location || 'LX1') + '</td>' +
                      '<td style="text-align: center; font-family: monospace; font-weight: bold;">' + (f.universe || 1) + '</td>' +
                      '<td style="text-align: center; font-family: monospace; font-weight: bold; color: #0284c7;">' + addr + '</td>' +
                      '<td style="text-align: center; font-family: monospace; font-weight: bold;">' + end + '</td>' +
                      '<td style="text-align: center; font-family: monospace;">' + ch + '</td>' +
                    '</tr>'
                  );
                }).join('')
              ) : '<tr><td colspan="8" style="text-align:center;color:#64748b;padding:12px;">No DMX lighting fixtures patched.</td></tr>') +
            '</tbody>' +
          '</table>'
      }
    ];
    ...
    if (univKeys.length) {
      sections.push({
        title: 'UNIVERSE CAPACITY & FOOTPRINT SUMMARY',
        html:
          '<table>' +
            '<thead><tr><th>Universe</th><th>Fixtures Patched</th><th>Channels Used</th><th>Available Capacity</th><th>Address Range Used</th></tr></thead>' +
            '<tbody>' +
              univKeys.map((u) => {
                const s = val.universeSummaries[u];
                return (
                  '<tr>' +
                    '<td style="font-weight: bold; font-family: monospace;">Universe ' + u + '</td>' +
                    '<td>' + s.fixtureCount + ' fixtures</td>' +
                    '<td style="font-weight: bold; color: #0284c7;">' + s.totalChannelsUsed + ' / 512 channels (' + Math.round((s.totalChannelsUsed / 512) * 100) + '%)</td>' +
                    '<td>' + (512 - s.totalChannelsUsed) + ' channels free</td>' +
                    '<td style="font-family: monospace;">Ch ' + s.minAddress + ' \u2013 ' + s.maxAddress + '</td>' +
                  '</tr>'
                );
              }).join('') +
            '</tbody>' +
    ...
    openPrintDocument({
      title: title || 'DMX Lighting Patch Sheet',
      subtitle: 'Lighting Rig Architecture & DMX Universe Assignments',
      meta: meta || [],
      sections: sections
    });
  }

  function renderSaveRoomIOModal(sheet) {
    const m = ui.modal('Save Room I/O Preset', 
      '<div class="flex flex-col gap-4 max-w-md">' +
        '<div class="text-sm text-muted">Save the current Home Run matrix, output patching, and connected stageboxes as a reusable preset for a specific room or venue space. Repatch layers and act assignments are not saved.</div>' +
        '<div class="grid gap-1.5">' +
          '<label class="text-[10px] uppercase font-bold text-muted">Room / Space Name</label>' +
          '<input type="text" id="rio-space-name" class="field" value="' + ui.esc(sheet.space || 'The Stage') + '" placeholder="e.g. The Stage, Main Hall" />' +
        '</div>' +
        '<div class="flex justify-end gap-2 mt-4 pt-4 border-t border-line">' +
          '<button type="button" id="rio-cancel" class="btn btn-ghost">Cancel</button>' +
          '<button type="button" id="rio-save" class="btn btn-primary">Save I/O Preset</button>' +
        '</div>' +
      '</div>'
    );
    
    m.root.querySelector('#rio-cancel').onclick = m.close;
    m.root.querySelector('#rio-save').onclick = () => {
      const spaceName = m.root.querySelector('#rio-space-name').value.trim();
      if (!spaceName) return ui.toast('Please enter a room name', 'error');
      
      // Filter out repatch layers, only save original physical sockets (layer 0)
      const presetBoxes = sheet.stageboxes.map(box => {
        const clonedBox = JSON.parse(JSON.stringify(box));
        clonedBox.channels = clonedBox.channels.filter(ch => !ch.repatch);
        return clonedBox;
      });
      
      saveRoomIOPreset({
        space: spaceName,
        homeRun: sheet.homeRun,
        stageboxes: presetBoxes
      });
      
      ui.toast('Saved Room I/O Preset: ' + spaceName, 'ok');
      m.close();
    };
  }
...
RMTP.views.presets = function (contentEl) {
  ...
  // 4. 'dmx'     -> DMX Lighting Patch Sheet & Fixture Engine
  let activeMainView = (RMTP._presetsMainView !== undefined ? RMTP._presetsMainView : 'sheets');
  let activeFilterTab = 'all'; // 'all', 'input', 'output'
  let activeDmxUniverse = 'all'; // 'all', '1', '2', etc.
  let searchQuery = '';

  // Built-in Lighting Rig Presets / Templates
  const BUILTIN_DMX_TEMPLATES = [
    {
      id: 'tmpl-stage-standard',
      name: 'The Stage — Standard House Rig (8 Fixtures)',
      space: 'The Stage',
      notes: '3x Spiider Wash, 2x Pointe Beams, 2x COLORado Pars, 1x ETC Lustr Key',
      fixtures: [
        { id: 'f-1', unit: 1, personalityId: 'fix-robe-spiider-27', manufacturer: 'Robe', model: 'Spiider', mode: 'Mode 1 (Standard Wash/Beam)', channels: 27, name: 'Back Wash SL', location: 'LX1', universe: 1, address: 1 },
        { id: 'f-2', unit: 2, personalityId: 'fix-robe-spiider-27', manufacturer: 'Robe', model: 'Spiider', mode: 'Mode 1 (Standard Wash/Beam)', channels: 27, name: 'Back Wash CS', location: 'LX1', universe: 1, address: 28 },
        { id: 'f-3', unit: 3, personalityId: 'fix-robe-spiider-27', manufacturer: 'Robe', model: 'Spiider', mode: 'Mode 1 (Standard Wash/Beam)', channels: 27, name: 'Back Wash SR', location: 'LX1', universe: 1, address: 55 },
        { id: 'f-4', unit: 4, personalityId: 'fix-robe-pointe-16', manufacturer: 'Robe', model: 'Pointe', mode: 'Mode 1 (Standard Beam/Spot)', channels: 16, name: 'Mid Beam SL', location: 'LX2', universe: 1, address: 82 },
        { id: 'f-5', unit: 5, personalityId: 'fix-robe-pointe-16', manufacturer: 'Robe', model: 'Pointe', mode: 'Mode 1 (Standard Beam/Spot)', channels: 16, name: 'Mid Beam SR', location: 'LX2', universe: 1, address: 98 },
        { id: 'f-6', unit: 6, personalityId: 'fix-chauvet-colorado-1-quad-12', manufacturer: 'Chauvet Professional', model: 'COLORado 1-Quad Zoom', mode: 'TOUR (12ch)', channels: 12, name: 'Side Warm SL', location: 'LX3', universe: 1, address: 114 },
        { id: 'f-7', unit: 7, personalityId: 'fix-chauvet-colorado-1-quad-12', manufacturer: 'Chauvet Professional', model: 'COLORado 1-Quad Zoom', mode: 'TOUR (12ch)', channels: 12, name: 'Side Warm SR', location: 'LX3', universe: 1, address: 126 },
        { id: 'f-8', unit: 8, personalityId: 'fix-etc-s4-led-s3-10', manufacturer: 'ETC', model: 'Source Four LED Series 3', mode: 'Lustr X8 Direct (10ch)', channels: 10, name: 'FOH Key Special CS', location: 'FOH Truss', universe: 1, address: 138 }
      ]
    },
    {
      id: 'tmpl-studio-acoustic',
      name: 'Studio 1 / Studio 2 — Acoustic & Intimate Rig (6 Fixtures)',
      space: 'Studio 1',
      notes: '4x COLORado Pars, 2x ETC Source 4 Lustr profiles',
      fixtures: [
        { id: 'f-1', unit: 1, personalityId: 'fix-chauvet-colorado-1-quad-12', manufacturer: 'Chauvet Professional', model: 'COLORado 1-Quad Zoom', mode: 'TOUR (12ch)', channels: 12, name: 'Warm Wash SL', location: 'Overhead 1', universe: 1, address: 1 },
        { id: 'f-2', unit: 2, personalityId: 'fix-chauvet-colorado-1-quad-12', manufacturer: 'Chauvet Professional', model: 'COLORado 1-Quad Zoom', mode: 'TOUR (12ch)', channels: 12, name: 'Warm Wash CS', location: 'Overhead 1', universe: 1, address: 13 },
        { id: 'f-3', unit: 3, personalityId: 'fix-chauvet-colorado-1-quad-12', manufacturer: 'Chauvet Professional', model: 'COLORado 1-Quad Zoom', mode: 'TOUR (12ch)', channels: 12, name: 'Warm Wash SR', location: 'Overhead 1', universe: 1, address: 25 },
        { id: 'f-4', unit: 4, personalityId: 'fix-chauvet-colorado-1-quad-12', manufacturer: 'Chauvet Professional', model: 'COLORado 1-Quad Zoom', mode: 'TOUR (12ch)', channels: 12, name: 'Floor Uplight CS', location: 'Floor USC', universe: 1, address: 37 },
        { id: 'f-5', unit: 5, personalityId: 'fix-etc-s4-led-s3-10', manufacturer: 'ETC', model: 'Source Four LED Series 3', mode: 'Lustr X8 Direct (10ch)', channels: 10, name: 'Key Light Left', location: 'FOH Bar', universe: 1, address: 49 },
        { id: 'f-6', unit: 6, personalityId: 'fix-etc-s4-led-s3-10', manufacturer: 'ETC', model: 'Source Four LED Series 3', mode: 'Lustr X8 Direct (10ch)', channels: 10, name: 'Key Light Right', location: 'FOH Bar', universe: 1, address: 59 }
      ]
    },
    {
      id: 'tmpl-club-multi-univ',
      name: 'Club & Live FX Rig — 2 Universes (8 Fixtures + Tubes)',
      space: 'The Stage',
      notes: 'Universe 1: Moving heads & Stage Wash. Universe 2: Astera Titan pixel tubes.',
      fixtures: [
        { id: 'f-1', unit: 1, personalityId: 'fix-robe-spiider-27', manufacturer: 'Robe', model: 'Spiider', mode: 'Mode 1 (Standard Wash/Beam)', channels: 27, name: 'Wash Back SL', location: 'LX1', universe: 1, address: 1 },
        { id: 'f-2', unit: 2, personalityId: 'fix-robe-spiider-27', manufacturer: 'Robe', model: 'Spiider', mode: 'Mode 1 (Standard Wash/Beam)', channels: 27, name: 'Wash Back CS', location: 'LX1', universe: 1, address: 28 },
        { id: 'f-3', unit: 3, personalityId: 'fix-robe-spiider-27', manufacturer: 'Robe', model: 'Spiider', mode: 'Mode 1 (Standard Wash/Beam)', channels: 27, name: 'Wash Back SR', location: 'LX1', universe: 1, address: 55 },
        { id: 'f-4', unit: 4, personalityId: 'fix-robe-pointe-16', manufacturer: 'Robe', model: 'Pointe', mode: 'Mode 1 (Standard Beam/Spot)', channels: 16, name: 'Spot Mid SL', location: 'LX2', universe: 1, address: 82 },
        { id: 'f-5', unit: 5, personalityId: 'fix-robe-pointe-16', manufacturer: 'Robe', model: 'Pointe', mode: 'Mode 1 (Standard Beam/Spot)', channels: 16, name: 'Spot Mid SR', location: 'LX2', universe: 1, address: 98 },
        { id: 'f-6', unit: 6, personalityId: 'fix-astera-titan-16px', manufacturer: 'Astera', model: 'Titan Tube (FP1)', mode: '16 Pixels RGB (48ch)', channels: 48, name: 'Titan Tube 1', location: 'Floor SL', universe: 2, address: 1 },
        { id: 'f-7', unit: 7, personalityId: 'fix-astera-titan-16px', manufacturer: 'Astera', model: 'Titan Tube (FP1)', mode: '16 Pixels RGB (48ch)', channels: 48, name: 'Titan Tube 2', location: 'Floor CS', universe: 2, address: 49 },
        { id: 'f-8', unit: 8, personalityId: 'fix-astera-titan-16px', manufacturer: 'Astera', model: 'Titan Tube (FP1)', mode: '16 Pixels RGB (48ch)', channels: 48, name: 'Titan Tube 3', location: 'Floor SR', universe: 2, address: 97 }
      ]
    }
  ];

  // Initialize or restore active DMX Lighting Patch State
  if (!RMTP._dmxPatchState) {
    const saved = (store && typeof store.read === 'function') ? store.read('dmx_active_patch', null) : null;
    if (saved && Array.isArray(saved.fixtures)) {
      RMTP._dmxPatchState = saved;
    } else {
      RMTP._dmxPatchState = JSON.parse(JSON.stringify(BUILTIN_DMX_TEMPLATES[0]));
      RMTP._dmxPatchState.title = 'Main Stage Lighting Rig Patch';
      RMTP._dmxPatchState.date = '';
      RMTP._dmxPatchState.eventId = '';
      RMTP._dmxPatchState.eventName = '';
    }
  }

  function getDmxPatch() {
    return RMTP._dmxPatchState;
  }
  ...
  function render() {
    ...
    const dmxPatch = getDmxPatch();
    const dmxFixtures = Array.isArray(dmxPatch.fixtures) ? dmxPatch.fixtures : [];
    ...
    contentEl.innerHTML =
      '<div class="grid gap-6">' +
        ...
        // 4 Primary Menu Options Tab Navigation
        '<div class="panel p-2 flex flex-wrap items-center justify-between gap-3 bg-panel2/60 border border-line">' +
          '<div class="flex items-center gap-1.5 flex-wrap">' +
            ...
            '</button>' +
            '<button data-main-view="dmx" class="px-4 py-2 rounded-lg text-xs font-bold transition-colors flex items-center gap-2 ' + (activeMainView === 'dmx' ? 'bg-accent text-accent-ink shadow-xs' : 'text-muted hover:text-ink hover:bg-panel') + '">' +
              ui.icon('bulb', 'w-4 h-4') +
              '<span>4. DMX Lighting Patch (' + dmxFixtures.length + ')</span>' +
            '</button>' +
          '</div>' +
          (activeMainView === 'sheets' || activeMainView === 'library' ? (
            ...
            '<div class="flex items-center gap-2 flex-wrap">' +
              '<button id="btn-top-dmx-export-csv" class="btn btn-ghost text-xs font-semibold flex items-center gap-1 border border-line text-ink hover:text-accent">' +
                ui.icon('download', 'w-3.5 h-3.5') + '<span>Export CSV</span>' +
              '</button>' +
              '<button id="btn-top-dmx-print" class="btn btn-primary text-xs font-semibold flex items-center gap-1.5 shadow-sm">' +
                ui.icon('print', 'w-3.5 h-3.5') + '<span>Print DMX Patch PDF</span>' +
              '</button>' +
            '</div>'
          ) : (
            '<div class="flex items-center gap-2 flex-wrap">' +
        ...
        // VIEW 4: DMX LIGHTING PATCH SHEET & RIG MANAGER
        (activeMainView === 'dmx' ? (() => {
          const val = RMTP.dmx ? RMTP.dmx.validatePatch(dmxFixtures) : { hasErrors: false, conflictMap: {}, universeSummaries: {}, overlaps: [], outOfBounds: [] };
          const univSummaries = val.universeSummaries || {};
          const univKeys = Object.keys(univSummaries).sort((a, b) => +a - +b);
          const personalities = RMTP.dmx ? RMTP.dmx.getAllPersonalities() : [];

          const groupedPersonalities = {};
          personalities.forEach((p) => {
            const mfg = p.manufacturer || 'Other';
            if (!groupedPersonalities[mfg]) groupedPersonalities[mfg] = [];
            groupedPersonalities[mfg].push(p);
          });

          const filteredFixtures = dmxFixtures.filter((f) => {
            if (activeDmxUniverse !== 'all' && String(f.universe || 1) !== String(activeDmxUniverse)) return false;
            return true;
          });

          const savedPatches = (RMTP.dmx && typeof RMTP.dmx.getAllPatches === 'function') ? RMTP.dmx.getAllPatches() : [];
          const advancingEvents = (store && typeof store.all === 'function') ? store.all('advancing') : [];

          return (
            '<div class="grid gap-5">' +
              // Rig Metadata & Templates Bar
              '<div class="panel p-4 border border-line bg-panel2/40 flex flex-wrap items-center justify-between gap-3 shadow-2xs">' +
                '<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 flex-1 min-w-[300px]">' +
                  '<div>' +
                    '<label class="block text-[11px] font-semibold text-muted mb-1">Patch Title / Rig Name</label>' +
                    '<input id="dmx-meta-title" class="field text-xs w-full bg-panel" value="' + ui.esc(dmxPatch.title || '') + '" placeholder="e.g. Main Stage Lighting Rig" />' +
                  '</div>' +
                  '<div>' +
                    '<label class="block text-[11px] font-semibold text-muted mb-1">Venue / Space</label>' +
                    '<select id="dmx-meta-space" class="field text-xs w-full bg-panel">' +
                      '<option value="The Stage" ' + (dmxPatch.space === 'The Stage' ? 'selected' : '') + '>The Stage</option>' +
                      '<option value="Studio 1" ' + (dmxPatch.space === 'Studio 1' ? 'selected' : '') + '>Studio 1</option>' +
                      '<option value="Studio 2" ' + (dmxPatch.space === 'Studio 2' ? 'selected' : '') + '>Studio 2</option>' +
                      '<option value="Outdoor / Other" ' + (dmxPatch.space === 'Outdoor / Other' ? 'selected' : '') + '>Outdoor / Other</option>' +
                    '</select>' +
                  '</div>' +
                  '<div>' +
                    '<label class="block text-[11px] font-semibold text-muted mb-1">Associate Event</label>' +
                    '<select id="dmx-meta-event" class="field text-xs w-full bg-panel">' +
                      '<option value="">(None / General Rig)</option>' +
                      advancingEvents.map((ev) => '<option value="' + ev.id + '" ' + (dmxPatch.eventId === ev.id ? 'selected' : '') + '>' + ui.esc(ev.name || 'Untitled') + ' (' + ui.esc(ev.date || '') + ')</option>').join('') +
                    '</select>' +
                  '</div>' +
                  '<div>' +
                    '<label class="block text-[11px] font-semibold text-muted mb-1">Date</label>' +
                    '<input id="dmx-meta-date" type="date" class="field text-xs w-full bg-panel" value="' + ui.esc(dmxPatch.date || '') + '" />' +
                  '</div>' +
                  '<div>' +
                    '<label class="block text-[11px] font-semibold text-muted mb-1">Load Rig Preset / DB</label>' +
                    '<select id="sel-dmx-load-template" class="field text-xs w-full bg-panel font-semibold text-accent">' +
                      '<option value="">Select Rig Preset\u2026</option>' +
                      (savedPatches.length ? (
                        '<optgroup label="Saved Cloud / DB Patches">' +
                          savedPatches.map((p) => '<option value="db:' + p.id + '">' + ui.esc(p.title || p.name || 'DMX Patch') + ' (' + (p.fixtures ? p.fixtures.length : 0) + ' fixtures)</option>').join('') +
                        '</optgroup>'
                      ) : '') +
                      '<optgroup label="Built-in Venue House Rigs">' +
                        BUILTIN_DMX_TEMPLATES.map((t) => '<option value="tmpl:' + t.id + '">' + ui.esc(t.name) + '</option>').join('') +
                      '</optgroup>' +
                    '</select>' +
                  '</div>' +
                '</div>' +
                '<div class="flex items-center gap-2 flex-wrap">' +
                  '<button type="button" id="btn-dmx-save-db" class="btn btn-primary text-xs flex items-center gap-1 font-semibold shadow-xs" title="Save this DMX rig to the database / event">' +
                    ui.icon('save', 'w-3.5 h-3.5') + '<span>Save Patch to DB</span>' +
                  '</button>' +
                  '<button type="button" id="btn-dmx-manage-lib" class="btn btn-ghost text-xs flex items-center gap-1 border border-line text-ink hover:text-accent font-semibold" title="Personalities Library">' +
                    ui.icon('sliders', 'w-3.5 h-3.5') + '<span>Personalities</span>' +
                  '</button>' +
                  '<button type="button" id="btn-dmx-clear-rig" class="btn btn-ghost text-xs text-muted hover:text-danger flex items-center gap-1" title="Clear rig">' +
                    ui.icon('trash', 'w-3.5 h-3.5') + '<span>Clear</span>' +
                  '</button>' +
                '</div>' +
              '</div>' +

              // Universe Metric Cards & Filter Bar
              '<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">' +
                '<div class="panel p-3.5 bg-panel border border-line flex items-center justify-between shadow-2xs">' +
                  '<div>' +
                    '<div class="text-[11px] text-muted uppercase font-bold tracking-wider">Total Patched</div>' +
                    '<div class="text-xl font-bold font-mono text-ink mt-0.5">' + dmxFixtures.length + ' <span class="text-xs font-normal text-muted">Fixtures</span></div>' +
                  '</div>' +
                  '<div class="w-10 h-10 rounded-lg bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 flex items-center justify-center">' +
                    ui.icon('bulb', 'w-5 h-5') +
                  '</div>' +
                '</div>' +

                '<div class="panel p-3.5 bg-panel border border-line flex items-center justify-between shadow-2xs">' +
                  '<div>' +
                    '<div class="text-[11px] text-muted uppercase font-bold tracking-wider">Active Universes</div>' +
                    '<div class="text-xl font-bold font-mono text-ink mt-0.5">' + (univKeys.length || 1) + ' <span class="text-xs font-normal text-muted">Universes</span></div>' +
                  '</div>' +
                  '<div class="w-10 h-10 rounded-lg bg-indigo-500/10 text-indigo-400 border border-indigo-500/30 flex items-center justify-center font-bold font-mono text-sm">' +
                    '512' +
                  '</div>' +
                '</div>' +
                ...
                '<div class="panel p-3.5 bg-panel border border-line flex items-center justify-between shadow-2xs">' +
                  '<div>' +
                    '<div class="text-[11px] text-muted uppercase font-bold tracking-wider">Address Conflicts</div>' +
                    '<div class="text-xl font-bold font-mono ' + (val.hasErrors ? 'text-rose-400' : 'text-emerald-400') + ' mt-0.5">' +
                      (val.hasErrors ? (val.overlaps.length + val.outOfBounds.length) + ' Conflicts' : '0 Clean') +
                    '</div>' +
                  '</div>' +
                  '<div class="w-10 h-10 rounded-lg ' + (val.hasErrors ? 'bg-rose-500/10 text-rose-400 border border-rose-500/30' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30') + ' flex items-center justify-center">' +
                    ui.icon(val.hasErrors ? 'warning' : 'check', 'w-5 h-5') +
                  '</div>' +
                '</div>' +

                '<div class="panel p-3.5 bg-panel border border-line flex items-center justify-between shadow-2xs">' +
                  '<div>' +
                    '<div class="text-[11px] text-muted uppercase font-bold tracking-wider">Auto-Addressing</div>' +
                    '<div class="text-xs text-muted mt-1">Recalculate gapless addresses</div>' +
                  '</div>' +
                  '<button type="button" id="btn-dmx-auto-addr-top" class="btn btn-ghost !py-1.5 !px-2.5 text-xs text-accent font-semibold border border-accent/40 hover:bg-accent/10 flex items-center gap-1 shadow-2xs">' +
                    ui.icon('zap', 'w-3.5 h-3.5 text-warning') + '<span>Auto-Address</span>' +
                  '</button>' +
                '</div>' +
              '</div>' +

              // Conflict Diagnostics Banner (if errors exist)
              (val.hasErrors ? (
                '<div class="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/40 text-xs text-rose-300 grid gap-2 shadow-2xs">' +
                  '<div class="flex items-center gap-2 font-bold text-rose-400">' +
                    ui.icon('warning', 'w-4 h-4 shrink-0') +
                    '<span>DMX Address Conflicts & Overlaps Detected (' + (val.overlaps.length + val.outOfBounds.length) + ' issues)</span>' +
                  '</div>' +
                  '<ul class="list-disc list-inside space-y-1 text-[11px] text-rose-200/90 pl-1">' +
                    val.overlaps.map((o) => '<li><strong>Universe ' + o.universe + ':</strong> Unit #' + o.fix1.unit + ' (' + ui.esc(o.fix1.name || o.fix1.model) + ' \u00b7 ch ' + o.fix1.start + '\u2013' + o.fix1.end + ') overlaps with Unit #' + o.fix2.unit + ' (' + ui.esc(o.fix2.name || o.fix2.model) + ' \u00b7 ch ' + o.fix2.start + '\u2013' + o.fix2.end + ') on ch ' + o.overlapStart + '\u2013' + o.overlapEnd + '</li>').join('') +
                    val.outOfBounds.map((b) => '<li><strong>Universe ' + b.universe + ':</strong> Unit #' + b.unit + ' (' + ui.esc(b.name || b.model) + ') end address ' + b.end + ' exceeds maximum universe limit 512</li>').join('') +
                  '</ul>' +
                '</div>'
              ) : '') +

              // Main Fixtures Table Panel
              '<div class="panel p-5 border border-line bg-panel grid gap-4 shadow-sm">' +
                '<div class="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-line">' +
                  '<div class="flex items-center gap-2 flex-wrap">' +
                    '<span class="text-xs font-bold text-ink uppercase tracking-wider">Universe Filter:</span>' +
                    '<button data-dmx-univ-filter="all" class="px-2.5 py-1 rounded text-xs font-mono font-bold border transition-colors ' + (activeDmxUniverse === 'all' ? 'bg-accent text-accent-ink border-accent' : 'bg-panel2 text-muted border-line hover:text-ink') + '">All Universes (' + dmxFixtures.length + ')</button>' +
                    univKeys.map((u) => {
                      const s = univSummaries[u];
                      const isSel = String(activeDmxUniverse) === String(u);
                      return '<button data-dmx-univ-filter="' + u + '" class="px-2.5 py-1 rounded text-xs font-mono font-bold border transition-colors ' + (isSel ? 'bg-accent text-accent-ink border-accent' : 'bg-panel2 text-muted border-line hover:text-ink') + '">Univ ' + u + ' (' + s.totalChannelsUsed + '/512 ch)</button>';
                    }).join('') +
                  '</div>' +
                  '<div class="flex items-center gap-2 flex-wrap">' +
                    '<button type="button" id="btn-add-fixture-main" class="btn btn-primary !py-1.5 !px-3 text-xs font-semibold flex items-center gap-1.5 shadow-sm">' +
                      ui.icon('plus', 'w-4 h-4') + '<span>Add Lighting Fixture</span>' +
                    '</button>' +
                  '</div>' +
                '</div>' +

                (!filteredFixtures.length ? (
                  '<div class="p-10 rounded-xl bg-panel2/40 border border-dashed border-line text-center grid gap-3">' +
                    '<div class="w-12 h-12 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 flex items-center justify-center mx-auto">' +
                      ui.icon('bulb', 'w-6 h-6') +
                    '</div>' +
                    '<div>' +
                      '<div class="text-sm font-bold text-ink">No DMX Lighting Fixtures Patched</div>' +
                      '<div class="text-xs text-muted max-w-sm mx-auto mt-1">Add fixtures manually or load a pre-configured house rig preset above.</div>' +
                    '</div>' +
                    '<div class="flex items-center justify-center gap-2">' +
                      '<button type="button" id="btn-add-fixture-empty" class="btn btn-primary text-xs font-semibold flex items-center gap-1.5">' +
                        ui.icon('plus', 'w-3.5 h-3.5') + '<span>Add First Fixture</span>' +
                      '</button>' +
                    '</div>' +
                  '</div>'
                ) : (
                  '<datalist id="dmx-loc-presets">' +
                    (RMTP.dmx ? RMTP.dmx.STANDARD_LOCATIONS : ['LX1', 'LX2', 'LX3', 'FOH Truss', 'Floor SL', 'Floor SR', 'Balcony']).map((l) => '<option value="' + ui.esc(l) + '"></option>').join('') +
                  '</datalist>' +
                  '<div class="overflow-x-auto rounded-xl border border-line bg-panel">' +
                    '<table class="w-full text-left text-xs border-collapse min-w-[760px]">' +
                      '<thead>' +
                        '<tr class="border-b border-line bg-panel2/80 text-[11px] font-bold text-muted uppercase tracking-wider">' +
                          '<th class="p-2.5 w-16 text-center">Unit #</th>' +
                          '<th class="p-2.5 w-60">Personality / Model Profile</th>' +
                          '<th class="p-2.5">Custom Name / Purpose</th>' +
                          '<th class="p-2.5 w-40">Hanging Location</th>' +
                          '<th class="p-2.5 w-20 text-center">Univ</th>' +
                          '<th class="p-2.5 w-20 text-center">Start Addr</th>' +
                          '<th class="p-2.5 w-24 text-center">End / Footprint</th>' +
                          '<th class="p-2.5 w-28 text-right">Actions</th>' +
                        '</tr>' +
                      '</thead>' +
                      '<tbody class="divide-y divide-line/60 font-medium">' +
                        filteredFixtures.map((fix) => {
                          const idx = dmxFixtures.indexOf(fix);
                          ...
                          return (
                            '<tr class="hover:bg-panel2/40 transition-colors ' + (isConflict ? 'bg-rose-500/10' : '') + '">' +
                              // Unit Number
                              '<td class="p-2 text-center">' +
                                '<input type="text" data-dmx-idx="' + idx + '" data-dmx-fld="unit" class="field !py-1 !px-1 text-center font-mono font-bold text-xs" value="' + ui.esc(fix.unit !== undefined ? fix.unit : (idx + 1)) + '" placeholder="#" />' +
                              '</td>' +
                              // Personality Select
                              '<td class="p-2">' +
                                '<select data-dmx-idx="' + idx + '" data-dmx-fld="personalityId" class="field !py-1 !px-2 text-xs text-ink font-semibold w-full">' +
                                  '<option value="">Select Fixture Personality\u2026</option>' +
                                  Object.keys(groupedPersonalities).sort().map((mfg) => (
                                    '<optgroup label="' + ui.esc(mfg) + '">' +
                                      groupedPersonalities[mfg].map((p) => {
                                        const isSel = (p.id && p.id === fix.personalityId) || (p.model === fix.model && p.channels === fix.channels);
                                        return '<option value="' + p.id + '" ' + (isSel ? 'selected' : '') + '>' + ui.esc(p.model + ' (' + (p.mode || 'Standard') + ' \u00b7 ' + p.channels + 'ch)') + '</option>';
                                      }).join('') +
                                    '</optgroup>'
                                  )).join('') +
                                  '<option value="__new__">+ Create New Personality\u2026</option>' +
                                '</select>' +
                                (fix.mode ? '<div class="text-[10px] text-muted mt-0.5 font-mono truncate">' + ui.esc((fix.manufacturer ? fix.manufacturer + ' ' : '') + (fix.mode ? '\u00b7 ' + fix.mode : '')) + '</div>' : '') +
                              '</td>' +
                              // Custom Name / Purpose
                              '<td class="p-2">' +
                                '<input type="text" data-dmx-idx="' + idx + '" data-dmx-fld="name" class="field !py-1 !px-2 text-xs w-full" value="' + ui.esc(fix.name || '') + '" placeholder="e.g. Back Wash SL" />' +
                              '</td>' +
                              // Location
                              '<td class="p-2">' +
                                '<input type="text" list="dmx-loc-presets" data-dmx-idx="' + idx + '" data-dmx-fld="location" class="field !py-1 !px-2 text-xs w-full" value="' + ui.esc(fix.location || '') + '" placeholder="LX1 / FOH / Floor" />' +
                              '</td>' +
                              // Universe
                              '<td class="p-2 text-center">' +
                                '<input type="number" min="1" max="16" data-dmx-idx="' + idx + '" data-dmx-fld="universe" class="field !py-1 !px-1 text-center font-mono text-xs w-16 mx-auto" value="' + univ + '" />' +
                              '</td>' +
                              // Start Address
                              '<td class="p-2 text-center">' +
                                '<input type="number" min="1" max="512" data-dmx-idx="' + idx + '" data-dmx-fld="address" class="field !py-1 !px-1 text-center font-mono font-bold text-xs w-16 mx-auto ' + (isConflict ? '!border-rose-500 !text-rose-400' : '') + '" value="' + addr + '" />' +
                              '</td>' +
                              // End Address & Footprint
                              '<td class="p-2 text-center font-mono text-[11px]">' +
                                '<div class="font-bold ' + (end > 512 ? 'text-rose-400' : 'text-cyan-400') + '">' + end + '</div>' +
                                '<div class="text-[10px] text-muted">' + ch + 'ch</div>' +
                              '</td>' +
                              // Actions
                              '<td class="p-2 text-right">' +
                                '<div class="flex items-center justify-end gap-1">' +
                                  '<button type="button" data-dmx-dup="' + idx + '" class="btn btn-ghost !p-1 text-ink hover:text-accent" title="Duplicate Fixture (+1 Address)">' + ui.icon('plus', 'w-3 h-3') + '</button>' +
                                  '<button type="button" data-dmx-up="' + idx + '" class="btn btn-ghost !p-1 text-muted hover:text-ink" title="Move Up" ' + (idx === 0 ? 'disabled' : '') + '>' + ui.icon('arrowU', 'w-3 h-3') + '</button>' +
                                  '<button type="button" data-dmx-down="' + idx + '" class="btn btn-ghost !p-1 text-muted hover:text-ink" title="Move Down" ' + (idx === dmxFixtures.length - 1 ? 'disabled' : '') + '>' + ui.icon('arrowD', 'w-3 h-3') + '</button>' +
                                  '<button type="button" data-dmx-del="' + idx + '" class="btn btn-danger !p-1 text-danger" title="Remove Fixture">' + ui.icon('trash', 'w-3 h-3') + '</button>' +
                                '</div>' +
                              '</td>' +
                            '</tr>'
                          );
              ...
              // Bottom DMX Action Bar
              '<div class="panel p-4 border border-line bg-panel2/60 flex flex-wrap items-center justify-between gap-3">' +
                '<div class="text-xs text-muted">' +
                  'Generate printable DMX Lighting Patch PDFs or export tabular CSV schedules for grandMA, ChamSys, Avolites & ETC EOS consoles.' +
                '</div>' +
                '<div class="flex items-center gap-2 flex-wrap">' +
                  '<button type="button" id="btn-bottom-dmx-auto-addr" class="btn btn-ghost text-xs font-semibold border border-line text-ink hover:text-accent flex items-center gap-1">' +
                    ui.icon('zap', 'w-3.5 h-3.5 text-warning') + '<span>Auto-Address Sequential</span>' +
                  '</button>' +
                  '<button type="button" id="btn-bottom-dmx-export-csv" class="btn btn-ghost text-xs font-semibold border border-line text-ink hover:text-accent flex items-center gap-1">' +
                    ui.icon('download', 'w-3.5 h-3.5') + '<span>Export CSV</span>' +
                  '</button>' +
                  '<button type="button" id="btn-bottom-dmx-print" class="btn btn-primary text-xs font-bold flex items-center gap-1.5 shadow-sm">' +
                    ui.icon('print', 'w-4 h-4') + '<span>Print DMX Patch PDF</span>' +
                  '</button>' +
                '</div>' +
              '</div>' +
            '</div>'
    ...
    // VIEW 4: DMX Event Handlers
    if (activeMainView === 'dmx') {
      const dmxPatch = getDmxPatch();
      if (!Array.isArray(dmxPatch.fixtures)) dmxPatch.fixtures = [];

      const saveDmxPatchState = () => {
        if (store && typeof store.write === 'function') {
          store.write('dmx_active_patch', dmxPatch);
        }
      };

      // Universe Filter Buttons
      contentEl.querySelectorAll('[data-dmx-univ-filter]').forEach((btn) => {
        btn.addEventListener('click', () => {
          activeDmxUniverse = btn.getAttribute('data-dmx-univ-filter');
          render();
        });
      });

      // Metadata inputs
      const titleInp = contentEl.querySelector('#dmx-meta-title');
      if (titleInp) titleInp.addEventListener('input', () => { dmxPatch.title = titleInp.value; saveDmxPatchState(); });

      const spaceSel = contentEl.querySelector('#dmx-meta-space');
      if (spaceSel) spaceSel.addEventListener('change', () => { dmxPatch.space = spaceSel.value; saveDmxPatchState(); });

      const eventSel = contentEl.querySelector('#dmx-meta-event');
      if (eventSel) {
        eventSel.addEventListener('change', () => {
          dmxPatch.eventId = eventSel.value || null;
          if (dmxPatch.eventId && store) {
            const ev = store.find('advancing', dmxPatch.eventId);
            if (ev) {
              dmxPatch.eventName = ev.name || '';
              if (ev.space) dmxPatch.space = ev.space;
              if (ev.date) dmxPatch.date = ev.date;
              // If event has fixtures and current rig is empty, import them
              if (Array.isArray(ev.dmx_fixtures) && ev.dmx_fixtures.length && (!dmxPatch.fixtures || !dmxPatch.fixtures.length)) {
                dmxPatch.fixtures = JSON.parse(JSON.stringify(ev.dmx_fixtures));
                ui.toast('Imported DMX patch from ' + (ev.name || 'Event'), 'ok');
              }
            }
          } else {
            dmxPatch.eventName = '';
      ...
      const dateInp = contentEl.querySelector('#dmx-meta-date');
      if (dateInp) dateInp.addEventListener('input', () => { dmxPatch.date = dateInp.value; saveDmxPatchState(); });

      // Save to Database / Event
      const saveDbBtn = contentEl.querySelector('#btn-dmx-save-db');
      if (saveDbBtn) {
        saveDbBtn.addEventListener('click', () => {
          if (!RMTP.dmx) return;
          const patchTitle = (dmxPatch.title || 'Main Stage Lighting Rig').trim();
          const saved = RMTP.dmx.savePatch(dmxPatch);
          if (saved && saved.id) {
      ...
      // Load Template / Saved DB Patch
      const loadTmplSel = contentEl.querySelector('#sel-dmx-load-template');
      if (loadTmplSel) {
        loadTmplSel.addEventListener('change', () => {
          const val = loadTmplSel.value;
          if (!val) return;
          if (val.startsWith('db:')) {
            const patchId = val.slice(3);
            const savedPatch = RMTP.dmx ? RMTP.dmx.getPatch(patchId) : null;
            if (savedPatch) {
              dmxPatch.id = savedPatch.id;
              dmxPatch.title = savedPatch.title || savedPatch.name || 'DMX Patch';
              dmxPatch.space = savedPatch.space || 'The Stage';
              dmxPatch.date = savedPatch.date || '';
              dmxPatch.eventId = savedPatch.eventId || null;
              dmxPatch.eventName = savedPatch.eventName || '';
              dmxPatch.fixtures = JSON.parse(JSON.stringify(savedPatch.fixtures || []));
              saveDmxPatchState();
              ui.toast('Loaded database patch: ' + dmxPatch.title, 'ok');
              render();
            }
          } else if (val.startsWith('tmpl:')) {
            const tmplId = val.slice(5);
            const tmpl = BUILTIN_DMX_TEMPLATES.find((t) => t.id === tmplId);
            if (tmpl) {
              dmxPatch.title = tmpl.name;
              dmxPatch.space = tmpl.space || 'The Stage';
              dmxPatch.fixtures = JSON.parse(JSON.stringify(tmpl.fixtures));
              saveDmxPatchState();
              ui.toast('Loaded preset: ' + tmpl.name, 'ok');
              render();
            }
          }
        });
      ...
      // Manage Personalities Library
      const manageLibBtn = contentEl.querySelector('#btn-dmx-manage-lib');
      if (manageLibBtn && RMTP.dmx) {
        manageLibBtn.addEventListener('click', () => {
          RMTP.dmx.openPersonalitiesManagerModal(() => render());
        });
      }

      // Clear Rig
      const clearRigBtn = contentEl.querySelector('#btn-dmx-clear-rig');
      if (clearRigBtn) {
        clearRigBtn.addEventListener('click', () => {
          if (confirm('Clear all DMX lighting fixtures in this patch?')) {
            dmxPatch.fixtures = [];
            saveDmxPatchState();
            ui.toast('Cleared lighting patch', 'ok');
            render();
          }
        });
      }

      // Auto-address handlers
      const triggerAutoAddress = () => {
        if (!RMTP.dmx) return;
        dmxPatch.fixtures = RMTP.dmx.autoAddressSequential(dmxPatch.fixtures, 1, 1);
        saveDmxPatchState();
        ui.toast('Auto-addressed fixtures sequentially', 'ok');
        render();
      };

      const autoAddrTopBtn = contentEl.querySelector('#btn-dmx-auto-addr-top');
      if (autoAddrTopBtn) autoAddrTopBtn.addEventListener('click', triggerAutoAddress);

      const autoAddrBottomBtn = contentEl.querySelector('#btn-bottom-dmx-auto-addr');
      if (autoAddrBottomBtn) autoAddrBottomBtn.addEventListener('click', triggerAutoAddress);

      // Add Fixture Buttons
      const addFixHandler = () => {
        const persList = RMTP.dmx ? RMTP.dmx.getAllPersonalities() : [];
        const defaultPers = persList[0] || { id: 'custom-par', manufacturer: 'Generic', model: 'LED Par RGBW', mode: '4ch', channels: 4, category: 'LED Par' };

        // Determine next start address
        let nextUniv = 1;
        let nextAddr = 1;
        if (dmxPatch.fixtures.length > 0) {
          const lastFix = dmxPatch.fixtures[dmxPatch.fixtures.length - 1];
          nextUniv = parseInt(lastFix.universe, 10) || 1;
          const lastAddr = parseInt(lastFix.address, 10) || 1;
          const lastCh = parseInt(lastFix.channels, 10) || 1;
          nextAddr = lastAddr + lastCh;
          if (nextAddr > 512) {
            nextUniv++;
            nextAddr = 1;
          }
        ...
        dmxPatch.fixtures.push({
          id: 'fix-' + Date.now() + '-' + Math.random().toString(36).substr(2, 4),
          unit: dmxPatch.fixtures.length + 1,
          personalityId: defaultPers.id,
          manufacturer: defaultPers.manufacturer,
          model: defaultPers.model,
          mode: defaultPers.mode,
          channels: defaultPers.channels,
          name: defaultPers.model,
          location: 'LX1',
          universe: nextUniv,
          address: nextAddr
        });
      ...
      const addFixMainBtn = contentEl.querySelector('#btn-add-fixture-main');
      if (addFixMainBtn) addFixMainBtn.addEventListener('click', addFixHandler);

      const addFixEmptyBtn = contentEl.querySelector('#btn-add-fixture-empty');
      if (addFixEmptyBtn) addFixEmptyBtn.addEventListener('click', addFixHandler);

      // Live Fixture Row Input Event Handlers
      contentEl.querySelectorAll('[data-dmx-fld]').forEach((el) => {
        const idx = parseInt(el.getAttribute('data-dmx-idx'), 10);
        const fld = el.getAttribute('data-dmx-fld');
        const fix = dmxPatch.fixtures[idx];
        if (!fix) return;

        if (fld === 'personalityId') {
          el.addEventListener('change', () => {
            if (el.value === '__new__') {
              if (RMTP.dmx) {
                RMTP.dmx.openPersonalityModal(null, (newPers) => {
                  fix.personalityId = newPers.id;
                  fix.manufacturer = newPers.manufacturer;
                  fix.model = newPers.model;
                  fix.mode = newPers.mode;
                  fix.channels = newPers.channels;
                  if (!fix.name || fix.name === 'Fixture') fix.name = newPers.model;
                  saveDmxPatchState();
                  render();
                });
              }
            ...
            if (RMTP.dmx) {
              const p = RMTP.dmx.getPersonality(el.value);
              if (p) {
                fix.personalityId = p.id;
                fix.manufacturer = p.manufacturer;
                fix.model = p.model;
                fix.mode = p.mode;
                fix.channels = p.channels;
                if (!fix.name || fix.name === 'Fixture') fix.name = p.model;
                saveDmxPatchState();
                render();
              }
            }
          ...
          el.addEventListener('input', () => {
            fix.location = el.value;
            saveDmxPatchState();
          });
        } else if (fld === 'universe') {
          el.addEventListener('input', () => {
            fix.universe = Math.max(1, parseInt(el.value, 10) || 1);
            saveDmxPatchState();
            render();
          });
        } else if (fld === 'address') {
          el.addEventListener('input', () => {
            fix.address = Math.max(1, Math.min(512, parseInt(el.value, 10) || 1));
            saveDmxPatchState();
            render();
          });
        }
      ...
      // Fixture Row Actions (Duplicate, Move Up, Move Down, Delete)
      contentEl.querySelectorAll('[data-dmx-dup]').forEach((btn) => {
        btn.addEventListener('click', () => {
          const idx = parseInt(btn.getAttribute('data-dmx-dup'), 10);
          const src = dmxPatch.fixtures[idx];
          if (!src) return;

          const ch = Math.max(1, parseInt(src.channels, 10) || 1);
          let newAddr = (parseInt(src.address, 10) || 1) + ch;
          let newUniv = parseInt(src.universe, 10) || 1;
          if (newAddr > 512) {
            newUniv++;
            newAddr = 1;
          }
          ...
          dup.id = 'fix-' + Date.now() + '-' + Math.random().toString(36).substr(2, 4);
          dup.unit = (parseInt(src.unit, 10) || (idx + 1)) + 1;
          dup.universe = newUniv;
          dup.address = newAddr;

          dmxPatch.fixtures.splice(idx + 1, 0, dup);
          saveDmxPatchState();
          ui.toast('Duplicated fixture: ' + (dup.name || dup.model), 'ok');
          render();
        });
      });

      contentEl.querySelectorAll('[data-dmx-up]').forEach((btn) => {
        btn.addEventListener('click', () => {
          const idx = parseInt(btn.getAttribute('data-dmx-up'), 10);
          if (idx > 0) {
            const item = dmxPatch.fixtures.splice(idx, 1)[0];
            dmxPatch.fixtures.splice(idx - 1, 0, item);
            saveDmxPatchState();
            render();
          }
        });
      ...
      contentEl.querySelectorAll('[data-dmx-down]').forEach((btn) => {
        btn.addEventListener('click', () => {
          const idx = parseInt(btn.getAttribute('data-dmx-down'), 10);
          if (idx < dmxPatch.fixtures.length - 1) {
            const item = dmxPatch.fixtures.splice(idx, 1)[0];
            dmxPatch.fixtures.splice(idx + 1, 0, item);
            saveDmxPatchState();
            render();
      ...
      contentEl.querySelectorAll('[data-dmx-del]').forEach((btn) => {
        btn.addEventListener('click', () => {
          const idx = parseInt(btn.getAttribute('data-dmx-del'), 10);
          dmxPatch.fixtures.splice(idx, 1);
          saveDmxPatchState();
          render();
        });
      });

      // Export CSV & Print PDF Handlers
      const exportDmxCsvHandler = () => {
        if (RMTP.dmx) {
          RMTP.dmx.exportCsv(dmxPatch.fixtures, (dmxPatch.title || 'DMX_Patch_Sheet').replace(/[^a-zA-Z0-9_-]/g, '_') + '.csv');
        }
      };

      const topCsvBtn = contentEl.querySelector('#btn-top-dmx-export-csv');
      if (topCsvBtn) topCsvBtn.addEventListener('click', exportDmxCsvHandler);

      const bottomCsvBtn = contentEl.querySelector('#btn-bottom-dmx-export-csv');
      if (bottomCsvBtn) bottomCsvBtn.addEventListener('click', exportDmxCsvHandler);

      const printDmxPdfHandler = () => {
        const meta = [
          { label: 'PATCH / RIG TITLE', val: dmxPatch.title || 'DMX Lighting Patch Sheet' },
          { label: 'VENUE / SPACE', val: dmxPatch.space || '—' },
          { label: 'DATE', val: dmxPatch.date || '—' },
          { label: 'TOTAL FIXTURES', val: dmxPatch.fixtures.length + ' fixtures' }
        ];
        presetsApi.printDmxPatchSheet(dmxPatch.title || 'DMX Lighting Patch Sheet', meta, dmxPatch.fixtures);
      };

      const topPrintBtn = contentEl.querySelector('#btn-top-dmx-print');
      if (topPrintBtn) topPrintBtn.addEventListener('click', printDmxPdfHandler);

      const bottomPrintBtn = contentEl.querySelector('#btn-bottom-dmx-print');
      if (bottomPrintBtn) bottomPrintBtn.addEventListener('click', printDmxPdfHandler);
    }
  }
