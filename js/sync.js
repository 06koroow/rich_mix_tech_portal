...
RMTP.sync = (function () {
  ...
  const mappers = {
    ...
    advancing: {
      toFields: (r) => ({ Title: r.name || '', AppId: r.id, Category: r.category || '', Space: r.space || '', Date: r.date || '', Status: r.status || '', StartTime: r.startTime || '', FinishTime: r.finishTime || '', Soundcheck: r.soundcheck || '', Doors: r.doors || '', Curfew: r.curfew || '', TechniciansJson: J(r.technicians), ClientContact: r.clientContact || '', GuestEngineer: !!r.guestEngineer, TechInfo: r.techInfo || '', TechSpecUrl: (r.techSpec && r.techSpec.url) || '', TechSpecName: (r.techSpec && r.techSpec.name) || '' }),
      fromItem: (f) => ({ id: f.AppId, name: f.Title, category: f.Category, space: f.Space, date: dOnly(f.Date), status: f.Status, startTime: f.StartTime || '', finishTime: f.FinishTime || '', soundcheck: f.Soundcheck || '', doors: f.Doors || '', curfew: f.Curfew || '', technicians: P(f.TechniciansJson), clientContact: f.ClientContact || '', guestEngineer: bool(f.GuestEngineer), techInfo: f.TechInfo || '', techSpec: f.TechSpecUrl ? { url: f.TechSpecUrl, name: f.TechSpecName || 'Tech spec', size: 0 } : null, checklist: {} }),
    },
    reports: {
    ...
    users: {
      toFields: (r) => ({ Title: ((r.firstName || '') + ' ' + (r.lastName || '')).trim(), AppId: r.id, FirstName: r.firstName || '', LastName: r.lastName || '', Email: r.email || '', Position: r.position || '', Status: r.status || 'active', Admin: !!r.admin, Trainer: !!r.trainer }),
      fromItem: (f) => ({ id: f.AppId, firstName: f.FirstName || '', lastName: f.LastName || '', email: f.Email || '', position: f.Position || '', status: f.Status || 'active', admin: bool(f.Admin), trainer: bool(f.Trainer) }),
    },
    signoffs: {
